...
"""
This class will take a cvs file which contains daily, hourly or minutely high and low, open and close as well as other 
...
"""
def getVolume(fileReader, currency):
...
"""
def amihud(fileReader):

    extractStr(fileReader)

    extractFlt()

    amihud= calculateAmihud()

    expression = getAmihudExpression()

    printAmihud(amihud, expression)
...
"""
def printAmihud(amihud, expression):
    print('open')
    print(np.openFlt)
    print('----------------')
    print('close')
    ...
    print('----------------')
    print('expression')
    print(getAmihudExpression())
    print('----------------')
    print(getAmihudSum(expression))
    print('----------------')
    print(amihud)
    print('----------------')
...
def getAmihudExpression():
    ...
    expression = expression[~numpy.isnan(expression)]  # to remove all nan values ( caused by missing USD volume values)
    return expression
...
def extractFlt():
    for value in openStr:
        np.openFlt.append(float(value))
    for value in closeStr:
        np.closeFlt.append(float(value))
    for value in volumeUSDStr:
...
"""
#TODO make it work for all types of currencies not only USD!
def extractStr(fileReader):
    for item in fileReader:
        openStr.append(item['open'])
        closeStr.append(item['close'])
        volumeUSDStr.append(item['VolumeUSD'])
    openStr.remove('open')
    closeStr.remove('close')
    volumeUSDStr.remove('VolumeUSD')
